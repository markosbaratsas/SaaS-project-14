...
router.post('/get-user-questions-per-period/',
    ...
    function(req, res, next) {
        ...
        pool.query(
            `SELECT  count(*), to_char(DateAsked,'YYYY-MM-DD') as date 
                FROM question
                WHERE DateAsked >= to_timestamp($1/ 1000.0) AND DateAsked <= to_timestamp($2/ 1000.0) AND UserID = $3
                GROUP BY date
                ORDER BY count(*) DESC`,
            [date_from.getTime(), date_to.getTime(), user_id],
            (err, results) => {
                if (err) {
                    res.status(400);
                    return res.json({error: "Something went wrong..."});
...
router.post('/get-user-answers-per-period/',
    ...
    function(req, res, next) {
        ...
        pool.query(
            `SELECT  count(*), to_char(DateAnswered,'YYYY-MM-DD') as date 
                FROM answer
                WHERE DateAnswered >= to_timestamp($1/ 1000.0) AND DateAnswered <= to_timestamp($2/ 1000.0) AND UserID = $3
                GROUP BY date
                ORDER BY count(*) DESC`,
            [date_from.getTime(), date_to.getTime(), user_id],
            (err, results) => {
